...
def pca(X=np.array([]), no_dims=50):
    ...
    eigen_values, eigen_vectors = np.array(eigen_values), np.array(
        eigen_vectors)

    #Sort eigen values in descending order
    eigen_values_sorted = np.array(np.sort(eigen_values)[::-1])
    eigen_values_k = np.array(eigen_values_sorted[0:no_dims])

    #Initialize M properly
    M = np.empty((0, S.shape[0]))
    for i in eigen_values_k:
        M = np.append(
            M,
            eigen_vectors[:, np.where(eigen_values == i)[0][0]].reshape(
                (1, S.shape[0])),
            axis=0)
    M = M.T
    Y = np.matmul(X, M)
...
def load_data(dataset='mnist_subset.json'):
    ...
    mnist = np.vstack(
        (np.asarray(data_set['train'][0]), np.asarray(data_set['valid'][0]),
