        ...
        for j in range(S):
            if t == 0:
                ...
                for i in range(len(A)):
                    sum_i += A[i][j] * alpha[i][t - 1]
                alpha[j][t] = B[j][O[t]] * sum_i
        ...
        for j in range(S - 1, -1, -1):
            if t == N - 1:
                ...
                for i in range(A.shape[1]):
                    sum_i += beta[i][t + 1] * A[j][i] * B[i][O[t]]
                beta[j][t] = sum_i
...
def viterbi(pi, A, B, O):
  ...
  """
    ...
    N = len(O)
    delta = np.zeros([S, N])
    path_delta = np.zeros([S, N])
    for t in range(N):
        for j in range(S):
            if t == 0:
                delta[j][t] = pi[j] * B[j][O[t]]
                path_delta[j][t] = j
            else:
                delta_i = []
                for i in range(len(A)):
                    delta_i.append(delta[i][t - 1] * A[i][j] * B[j][O[t]])
                delta_i = np.array(delta_i)
                delta[j][t] = np.max(delta_i)
                path_delta[j][t] = np.argmax(delta_i)
    # Add the argmax from end column of delta
    path.append(int(path_delta[np.argmax(delta[:, N - 1])][N - 1]))
    cnt = 0
    for i in range(N - 2, -1, -1):
        path.append(int(path_delta[path[cnt]][i]))
        cnt += 1
