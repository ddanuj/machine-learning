...
def transform_image(image, code_vectors):
    ...
    dist=np.empty((image.shape[0],image.shape[1],len(code_vectors)))
    for k in range(len(code_vectors)):
        dist[:,:,k]=np.linalg.norm(np.subtract(code_vectors[k],image),axis=2)**2
    for i in range(len(image)):
        for j in range(len(image[i])):
            transformed_image[i][j] = code_vectors[np.argmin(dist[i][j])]
    '''raise Exception(
