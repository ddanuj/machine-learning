...
class linear_layer:
    ...
    def forward(self, X):
        ...
        #Add bias to W and 1 to X
        W = np.concatenate((self.params['b'],self.params['W']),axis=0)
        X = np.append(np.ones((X.shape[0],1)),X,axis=1)

        forward_output = np.matmul(X,W)
    ...
    def backward(self, X, grad):
        ...
        self.gradient['b'] = np.matmul(np.ones((1,grad.shape[0])),grad)
...
class relu:
    ...
    def backward(self, X, grad):
        ...
        # PLEASE follow the Heaviside step function defined in CSCI567_HW2.pdf                                                   #
        ##########################################################################################################################
        backward_output = grad * np.heaviside(X,0)
