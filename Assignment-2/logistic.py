...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    #Append bias to weight vector
    W = np.concatenate((np.array([b]),w),axis=0)

    #Append 1 column to x to account for bias
    X = np.append(np.ones((N,1)),X,axis=1)

    itr=0
    while itr < max_iterations:
        gradient = np.matmul(sigmoid(np.matmul(X,W.T)) - y,X)
        if (gradient == 0.0).all():
            break
        else:
...
def binary_predict(X, w, b):
    ...
    W = np.concatenate((np.array([b]),w),axis=0)
    X = np.append(np.ones((N,1)),X,axis=1)
    pred_values = sigmoid(np.matmul(X,W.T))
    for i in range(N):
...
def multinomial_train(X, y, C, 
    ...
    #Add bias column in w 
    W = np.append(np.reshape(b,(-1,1)),w,axis=1)
    #Add 1 column in x to account for bias
    X = np.append(np.ones((N,1)),X,axis=1)

    #Getting one - hot representation of the classification labels
    Y = np.zeros((N, C))
    for i in range(N):
        Y[i][y[i]] = 1
    ...
    itr = 0
    while itr < max_iterations:
        softmax_val = softmax(np.matmul(X,W.T)) - Y
        gradient = np.matmul(softmax_val.T,X)
        if (gradient == 0.0).all():
            break
        else:
            W -= (step_size * (gradient / N))
...
def multinomial_predict(X, w, b):
    ...
    preds_mat = softmax(np.matmul(X,W.T))
    for i in range(N):
        preds[i] = np.argmax(preds_mat[i])
        #print(preds[i])
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for i in range(C):
        ...
        for j in range(N):
            if y[j] == i:
                Y[j] = 1
        w[i], b[i] = binary_train(X,Y,w[i],b[i])
...
def OVR_predict(X, w, b):
    ...
    pred_arr = np.zeros((C,N))
    for i in range(C):
        pred_arr[i] = binary_predict(X,w[i],b[i])
    
    for i in range(N):
        preds[i] = np.argmax(pred_arr[:,i])
